...
class UKF_SNMPC:
    def __init__(self):
        ...
        # xd : number of states
        # xa : algebreic variables. But it looks like those are reaction rates
        # xu : uncertain parameters
        # u: inputs
        # ODEeq: rhs of ODE
        # Aeq: algebraic equations
        # Obj_M: mayer term
        # Obj_L: lagrange term
        # R: control change penalty
        # u_min, u_max : constraints on inputs
        # states: list with states names
        # algebraics: list with algebric names
        # inputs:list with inputs names
        # hfcn: measurement function
        # Sigma_v: mmeasurement noise covariance matrix
        # gpdef: path constraints as SX equaitions
        # pgp: list with probability of cosntraints violation
        # Sigma_w: variance of the state noise
        # ngt: number of terminal constraints
        # gtfcn: terminal constraints function
        # pgt: probability of terminal cosntraint violation
        # CovP: covariance matrix uncertain parameters
        # MeanP: mean uncertain parameters
        # nm: number of measurments
        # Meanx0: mean initial conditions
        # Covx0: covariance initial condition
        # MeanSEx0: mean of the previous state estimate
        # CovSEx0: covariance of the previous state estimate
        # u0: Previous control input and previous state estimate at t = 0
        # xD_init, xA_init, u_init: initialization values
        ...
        # tf: time horizon
        # nk: number of control interfals
        # deg: degree interpolation for collocation
        # cp: type of collocation points (defaults radau)
        # nicp: number of collocation points per control interval
        # opts: ipopt options
        # alfa, beta kappa: parameters of UKF
        # robust horizon: after this the uncertaint does not grow

        self.h = self.tf / self.nk / self.nicp  # interval
        self.nd, self.na = SX.size(self.xd)[0], SX.size(self.xa)[0]
        self.nu, self.nun = SX.size(self.u)[0], SX.size(self.xu)[0]
        self.n_L = self.nd + self.nun  # number of expanded states (state + unc parameters)
        ...
        # lambda_ukf: a parameter. See Van der Werbe paper
        # nu_m, nu_c,sqrt_scaling_factor,scaling_factor: other parameters See Van der Werbe paper
        # sqrt_CovP, sqrt_Sigma_w, sqrt_Sigma_v square root of these matrices (cholewsky based)
            ...
            for l in range(ns):
                ...
                for i in range(nicp):
                    ...
                    for j in range(1, deg + 1):
                        ...
                        xp_jk = 0
                        for j2 in range(deg + 1):
                            xp_jk += C[j2][j] * XD[k][i][j2][
                                l]  # get the time derivative of the differential states (eq 10.19b)
    ...
    def Unscented_transformation_m(self, nu_m, nu_c, nm, sqrt_Sigma_v, ns, XD, k, nicp, deg, hfcn):
        ...
        for i in range(ns): # Equation (8) of the Van der Merve paper
